...
func (r *GoddessMomentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			return res, fmt.Errorf("gm '%s' in work queue no longer exists", req.Name)
		}
	...
	if !ok {
		return res, fmt.Errorf("%s 朋友圈找不到 %s 的关注食物", goddessMoment.Name, focusFood)
	}
	...
	if !foundFocusFood {
		logger.Info("关注食物没有在朋友圈发现", "name", r.Name, "food", focusFood, "goddessmoment", req.Name)
		return res, nil
	}
	...
	if foodStatus == nil {
		return res, fmt.Errorf("关注食物 %s 在女神朋友圈出现, 但是在status里没有找到", focusFood)
	}

	logger.Info("朋友圈关注的食物状态是", "goddessMoment", goddessMoment.Name, "name", r.Name, "food", focusFood, "status", foodStatus.Status)
	...
	if foodStatus.Status == kbv1.FoodStatusPending {
		...
		if foodStatus.ClaimBy != r.Name {
			logger.Info("朋友圈食物已经被人认领了, 我来的太迟了", "goddessMoment", goddessMoment.Name, "food", focusFood, "name", foodStatus.ClaimBy)
			return res, nil
		}
		...
		//  状态是已到达, 本条朋友圈任务已完成
		logger.Info("朋友圈食物我已经认领并送到了, 可以不用关注了", "goddessMoment", goddessMoment.Name, "food", focusFood)
	}

	if err != nil {
		err = fmt.Errorf("处理朋友圈 %s 更新失败, err: %s", goddessMoment.Name, err.Error())
		res.Requeue = true
		return res, err
	}
...
func (r *GoddessMomentReconciler) handlePending(ctx context.Context, gm *kbv1.GoddessMoment, index int) error {
	...
	if r.Name == "小杰" {
		...
		// 使用update更新
		gmCopy := gm.DeepCopy()
		gmCopy.Status.FoodDemand[index].Status = kbv1.FoodStatusPendingArrival
		gmCopy.Status.FoodDemand[index].ClaimTime = metav1.Now()
		gmCopy.Status.FoodDemand[index].ClaimBy = r.Name

		err := r.Update(ctx, gmCopy)
		if err != nil {
			return fmt.Errorf("更新朋友圈 %s 告诉女神 %s 我来送失败, err: %s", gm.Name, focusFood, err.Error())
		}
	}

	logger.Info("更新朋友圈告诉女神送来成功", "goddessMoment", gm.Name, "food", focusFood)
	return nil
}
...
func (r *GoddessMomentReconciler) handlePendingArrival(ctx context.Context, gm *kbv1.GoddessMoment, index int) error {
	...
	logger.Info("为朋友圈购买需要秒, 正在前往购买", "goddessMoment", gm.Name, "food", focusFood, "buyFoodTime", buyFoodTimeCost/time.Second)

	gmCopy := gm.DeepCopy()
	gmCopy.Status.FoodDemand[index].Status = kbv1.FoodStatusArrived
